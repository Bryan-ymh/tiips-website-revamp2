...
var hasError = function (field) {
    ...
    if (validity.valueMissing) return '請填寫此範圍。';

    // If not the right type
    if (validity.typeMismatch) {

        // Email
        if (field.type === 'email') return '請輸入電子郵件地址。';

        // URL
        if (field.type === 'url') return '請輸入一個網址(URL)。';
    ...
    if (validity.badInput) return '請輸入一個數字。';

    // If a number value doesn't match the step interval
    if (validity.stepMismatch) return '請選擇一個有效資料。';

    // If a number field is over the max
    if (validity.rangeOverflow) return '您輸入的資料不可超於 ' + field.getAttribute('max') + '。';

    // If a number field is below the min
    if (validity.rangeUnderflow) return '您輸入的資料不可少於' + field.getAttribute('min') + '。';
...
document.addEventListener('submit', function (event) {
    ...
    console.log (fields)
    var aa= document.getElementById("mce-EMAIL").value 
    if ( aa !== "/^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/"){
        return
    }
